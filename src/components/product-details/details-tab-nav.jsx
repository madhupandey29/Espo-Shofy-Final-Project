...
/* ---------- Suitable For (2nd screenshot style: columns + bullets) ---------- */
function SuitableForGrid({ value }) {
  // normalize any input shape -> { Category: [items...] }
  const data = React.useMemo(() => {
    if (!value) return {};

    // already object map: { Menswear: [...], Womenswear: [...] }
    if (typeof value === 'object' && !Array.isArray(value)) return value;

    // array fallback: ["Casual shirts"] or [{v:"Casual shirts"}]
    if (Array.isArray(value)) {
      const items = value
        .map((x) => {
          if (x == null) return '';
          if (typeof x === 'string' || typeof x === 'number') return String(x);
          return String(x.v ?? x.value ?? x.name ?? '');
        })
        .map((s) => String(s).trim())
        .filter(Boolean);

      return items.length ? { General: items } : {};
    }
  ...
  const entries = React.useMemo(() => {
    return Object.entries(data)
      .map(([cat, items]) => [cat, Array.isArray(items) ? items.filter(Boolean) : []])
  ...
  return (
    <div className="suitable-grid">
      {entries.map(([category, items]) => {
        ...
        const showHeading = category && category !== 'General';

        return (
          <div key={category} className="suitable-col">
            {showHeading && <div className="suitable-title">{category}</div>}

            <ul className="suitable-list">
              {uniq.map((item) => (
                <li key={`${category}-${item}`} className="suitable-item">
                  {item}
                </li>
              ))}
            </ul>
      ...
      <style jsx>{`
        ...
        .suitable-list {
          margin: 0;
          padding-left: 18px; /* bullet indent like screenshot */
        }
...
function renderValue(value, fieldLabel) {
  // ✅ Always render Suitable For as column + bullets (like screenshot)
  if (fieldLabel === 'Suitable For') {
    return <SuitableForGrid value={value} />;
  }
...
export default function DetailsTabNav({ product = {} }) {
  ...
  /* ─── SUITABILITY resolve (parse from aiTempOutput if suitability is empty) ─── */
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (match) {
          ...
          if (category && prod) {
            if (!suitabilityByCategory[category]) suitabilityByCategory[category] = [];
            // ✅ dedupe
            if (!suitabilityByCategory[category].includes(prod)) {
              suitabilityByCategory[category].push(prod);
            }
          }
        }
      }
  ...
  const rawSuitability = product?.suitability ?? full?.suitability;
  const aiTempOutput = product?.aiTempOutput ?? full?.aiTempOutput;

  // Use suitability array if it has data, otherwise parse from aiTempOutput
  ...
  /* ─── SUBSUITABLE normalize (keep existing logic as fallback) ─── */
  ...
  /* ---------- WEIGHT (rounding + ozs support) ---------- */
  const gsmVal = pick(product?.gsm, full?.gsm);
  const ozVal  = pick(product?.ozs, product?.oz, full?.ozs, full?.oz);
  ...
  const rows = [
    ...
    // ✅ Suitable For now always renders in bullet-columns style via renderValue()
    { label: 'Suitable For', value: Object.keys(suitabilityData).length > 0
                                    ? suitabilityData
                                    : (subsuitableNames.length ? subsuitableNames.map(s => ({ v: s }))
                                    : (subsuitableNameFB ? [{ v: subsuitableNameFB }] : ['—'])) },
                  ...
                  <div className="specs-table">
                    {rows.map(({ label, value }) => (
                      <div key={label} className="spec-row-compact">
                        ...
                        <div className="spec-value-compact">{renderValue(value, label)}</div>
                      </div>
                    ))}
                  </div>
                </div>
