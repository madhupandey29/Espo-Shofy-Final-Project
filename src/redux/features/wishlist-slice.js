...
const getPid = (x) => x?.productId || x?._id || x?.id || x?.product?._id || x?.product || x;
  ...
  async ({ customerAccountId, product }, { getState, rejectWithValue }) => {
    try {
      ...
      if (existingItem) {
        ...
        const wishlistItemId = existingItem.wishlistItemId || existingItem.__originalWishlistItem?.id;
        ...
        return current.filter((it) => 
          (it.wishlistItemId || it.__originalWishlistItem?.id) !== wishlistItemId
        );
...
export const removeWishlistItem = createAsyncThunk(
  ...
  async ({ customerAccountId, productId, title }, { getState, rejectWithValue }) => {
    try {
      if (!customerAccountId) throw new Error("Not logged in");
      ...
      const item = current.find((it) => {
        ...
        console.log('🔍 Comparing:', itemProductId, '===', searchProductId, '?', itemProductId === searchProductId);
        return itemProductId === searchProductId;
      });
      
      if (!item) {
        console.error('❌ Item not found in wishlist. ProductId:', productId);
        console.error('Available items:', current.map(it => ({
      ...
      const wishlistItemId = item.wishlistItemId || item.__originalWishlistItem?.id;
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchWishlist.fulfilled, (state, { payload, meta }) => {
        ...
        console.log('✅ FETCH WISHLIST - Total items stored:', state.wishlist.length);
