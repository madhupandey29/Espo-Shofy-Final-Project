...
const FooterISR = ({ office: officeProp = null }) => {
  ...
  const officeAddress = office?.addressStreet && office?.addressCity
  ...
  const addresses = [
    ...
    { title: "Factory Address", address: "1, Mohan Estate, Ramol Road, Ahmedabad, Gujarat, India – 382449" },
    { title: "Warehouse Address", address: "Nr. Ambuja Synthetics, B/H Old Narol Court, Narol, Ahmedabad, Gujarat, India – 382405" },
    { title: "UAE Office Address", address: "GSK Worldwide FZE, Ajman Free Zone, UAE" },
  ];

  const socials = React.useMemo(() => {
    const items = [
      { key: "facebook", url: office?.facebookUrl, label: "Facebook", icon: <FaFacebookF size={18} /> },
      { key: "instagram", url: office?.instagramUrl, label: "Instagram", icon: <FaInstagram size={18} /> },
      { key: "linkedin", url: office?.linkedinUrl, label: "LinkedIn", icon: <FaLinkedinIn size={18} /> },
      { key: "twitter", url: office?.xUrl, label: "X", icon: <FaXTwitter size={18} /> },
      { key: "youtube", url: office?.youtubeUrl, label: "YouTube", icon: <FaYoutube size={20} /> },
      { key: "pinterest", url: office?.pinterestUrl, label: "Pinterest", icon: <FaPinterestP size={18} /> },
  ...
  const onSubscribe = async (e) => {
    ...
    try {
      ...
      const payload = {
        salutationName: "", firstName: "Newsletter", lastName: "Subscriber", middleName: "",
        emailAddress: em, phoneNumber: "",
        ...
        opportunityAmountCurrency: "USD", opportunityAmount: null,
        cBusinessType: ["newsletter"], cFabricCategory: [],
      ...
      if (err?.message?.includes('404')) errorMessage = 'Subscription service not found. Please contact support.';
  ...
  return (
    <footer aria-label="Site Footer" className="age-footer">
            ...
            <div className="age-grid" role="list">
                ...
                <div>
                  <div className="age-addressBoard" aria-label="Company addresses">
                    {addresses.map((a, idx) => {
                      ...
                      return (
                        <a key={a.title} href={mapLink(a.address)} target="_blank" rel="noopener noreferrer"
                          className={`age-addressSection ${noSep ? "age--noSep" : ""} age-addressLink`}
                          aria-label={`${a.title} (Open in Google Maps)`} title="Open in Google Maps">
              ...
              <div className="age-col" role="listitem">
                <div className="age-widget">
                  ...
                  <ul className="age-list">
                    <li><Link href="/about" className="age-link">About</Link></li>
                    <li><Link href="/capabilities" className="age-link">Capabilities</Link></li>
                    <li><Link href="/fabric" className="age-link">Products</Link></li>
                    <li><Link href="/blog" className="age-link">Blog</Link></li>
                    <li><Link href="/contact" className="age-link">Contact</Link></li>
                  </ul>
                </div>
              </div>
              ...
              <div className="age-col" role="listitem">
                <div className="age-widget">
                  ...
                  <ul className="age-list">
                    <li><Link href="/wishlist" className="age-link">Wishlist</Link></li>
                    <li><Link href="/cart" className="age-link">Shopping Cart</Link></li>
                    <li><Link href="/profile" className="age-link">My Account</Link></li>
                    <li><Link href="/sitemap" className="age-link">Sitemap</Link></li>
                  </ul>
                </div>
              </div>
                ...
                <div className="age-widget">
                  ...
                  <div className="age-newsMini">
                    ...
                    <form className="age-pill" onSubmit={onSubscribe}>
                      ...
                      <button type="submit" aria-label="Subscribe" className="age-pillBtn" disabled={isSubscribing}
                  ...
                  <div className="age-talk">
                    {!!phone1Digits && (
                      <div className="age-talkRow">
                        ...
                        <a className="age-talkLink" href={`tel:+${phone1Digits}`}>
                          {phone1Raw || `+${phone1Digits}`}{phone1Dept && ` (${phone1Dept})`}
                        </a>
                      </div>
                    )}
                    {!!phone2Digits && phone2Digits !== phone1Digits && (
                      <div className="age-talkRow">
                        ...
                        <a className="age-talkLink" href={`tel:+${phone2Digits}`}>
                          {phone2Raw || `+${phone2Digits}`}{phone2Dept && ` (${phone2Dept})`}
                        </a>
                      </div>
                    )}
                    {salesEmail && (
                      <div className="age-talkRow">
                        ...
                        <div>
                          <div style={{ fontSize: "12px", color: "#D6A74B", fontWeight: "600", marginBottom: "2px" }}>Sales Email</div>
                          <a className="age-talkLink" href={mailTo(salesEmail, "Sales Inquiry", "Hi, I would like to know more about your products.")} title="Email Sales">{salesEmail}</a>
                        </div>
                      </div>
                    )}
                    {supportEmail && (
                      <div className="age-talkRow">
                        <span className="age-talkIcon"><Email /></span>
                        <div>
                          <div style={{ fontSize: "12px", color: "#D6A74B", fontWeight: "600", marginBottom: "2px" }}>Support Email</div>
                          <a className="age-talkLink" href={mailTo(supportEmail, "Support Request", "Hi, I need help with...")} title="Email Support">{supportEmail}</a>
                        </div>
                      </div>
                    )}
                    {!salesEmail && !supportEmail && primaryEmail && (
                      <div className="age-talkRow">
                        <span className="age-talkIcon"><Email /></span>
                        <a className="age-talkLink" href={mailTo(primaryEmail, "Inquiry", "Hi,")} title="Email Us">{primaryEmail}</a>
                      </div>
                    )}
                  </div>
                  ...
                  {officeAddress && (
                    <div className="age-talkAddress">
                      <a className="age-addrRow age-addrLink" href={mapLink(officeAddress)} target="_blank" rel="noopener noreferrer" aria-label="Open office address in Google Maps">
                        <span className="age-addrPin"><Location /></span>
                        <div className="age-addrLines">
                          {splitLines(officeAddress).slice(0, 4).map((l, i) => (<div key={i}>{l}</div>))}
                        </div>
                      </a>
                    </div>
                  )}
                  ...
                  <div className="age-social" role="group" aria-label="Social links">
                    {socials.map((s) => (
                      <a key={s.key} href={String(s.url)} target="_blank" rel="noopener noreferrer"
                        aria-label={s.label} title={s.label} className="age-socialBtn">{s.icon}</a>
                    ))}
                  </div>
                </div>
              </div>
          ...
          <div className="age-container">
            <div className="age-bottomWrap">
              ...
              <p className="age-copy">
                <strong>© {new Date().getFullYear()} {office?.legalName || office?.name || "Amrita Global Enterprises"}. All rights reserved under applicable laws.</strong>
              </p>
              <div className="age-trust">
                {trustedLogos.map((l, i) => (
                  <div key={i} className="age-trustCard" title={l.alt}>
                    <Image src={l.src} alt={l.alt} width={60} height={60} style={{ width: "36px", height: "auto", maxHeight: 36 }} />
                  </div>
                ))}
              </div>
            </div>
      ...
      <style jsx>{`
        ...
        .age-title{ font-family:'Poppins','Plus Jakarta Sans',system-ui,-apple-system,sans-serif; color:${TEXT_MAIN}; font-weight:800; font-size:20px; letter-spacing:.2px; margin:0 0 18px; position:relative; }
        ...
        .age-footer :global(a.age-link){ display:inline-block; font-weight:600; font-size:15px; text-decoration:none; color:${TEXT_MAIN} !important; border-bottom:1px dashed rgba(255,255,255,.25) !important; transition:color .22s ease, transform .15s ease, border-bottom-color .22s ease; }
        .age-footer :global(a.age-link:hover){ color:${BRAND_GOLD} !important; border-bottom-color:${BRAND_GOLD} !important; transform:translateX(3px); }
        .age-addressBoard{ background:linear-gradient(180deg,rgba(255,255,255,.06),rgba(255,255,255,.03)); border:1px solid ${BORDER_SOFT}; border-radius:16px; padding:16px 16px 8px; }
        ...
        .age-addressTitle{ font-family:'Poppins','Plus Jakarta Sans',system-ui; font-weight:800; letter-spacing:.2px; margin:0 0 6px; color:${TEXT_MAIN}; display:flex; align-items:center; gap:8px; }
        ...
        .age-social{ display:flex; align-items:center; margin-top:14px; width:100%; max-width:100%; flex-wrap:nowrap; overflow:hidden; gap:10px; }
        .age-socialBtn{ flex:0 0 auto; width:40px; height:40px; border-radius:11px; display:inline-flex; align-items:center; justify-content:center; color:${TEXT_MAIN}; border:1px solid ${BORDER_SOFT}; background:rgba(255,255,255,.06); transition:transform .15s ease, box-shadow .2s ease, background .2s; }
        ...
        @media (min-width: 1101px){ .age-social{ gap:6px; } .age-socialBtn{ width:36px; height:36px; border-radius:10px; } }
        @media (max-width: 420px){ .age-social{ gap:9px; } .age-socialBtn{ width:38px; height:38px; border-radius:10px; } }
        ...
        .age-trustCard{ width:52px; height:52px; border-radius:12px; background:#fff; border:1px solid rgba(16,24,40,.08); box-shadow:0 6px 16px rgba(0,0,0,.18), inset 0 1px 0 rgba(255,255,255,.7); display:flex; align-items:center; justify-content:center; transition:transform .16s ease, box-shadow .16s ease, filter .16s ease; }
        ...
        @media (max-width:991px){ .age-title{ font-size:18px; } .age-footer__top{ padding:40px 0 22px; } }
        @media (max-width:640px){ .age-container{ padding: 0 20px; } .age-widget{ padding: 5px 18px; } .age-title::after{ left:8%; transform:translateX(-50%); } .age-list li{ margin: 0 0 12px; } .age-addressBoard{ padding:16px 14px 10px; } .age-addressTitle{ justify-content:flex-start; } .age-talkRow{ justify-content:flex-start; } .age-addrRow{justify-content:flex-start; text-align:left; } .age-social{ justify-content:center; } }
        @media (max-width:575px){ .age-copy{ font-size:13.6px; } .age-trustCard{ width:42px; height:42px; border-radius:10px; } .age-trustCard :global(img){ max-height:30px; } }
        :global(.Toastify__toast){ border-radius: 14px !important; background: linear-gradient(180deg, rgba(17,35,56,.98), rgba(20,42,66,.98)) !important; color: ${TEXT_MAIN} !important; border: 1px solid rgba(255,255,255,.12) !important; box-shadow: 0 14px 36px rgba(0,0,0,.35) !important; backdrop-filter: blur(8px); }
        :global(.Toastify__toast-body){ font-family: 'Plus Jakarta Sans', system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif !important; font-weight: 700; letter-spacing: .15px; }
        :global(.Toastify__progress-bar){ background: linear-gradient(90deg, ${BRAND_GOLD}, ${BRAND_BLUE}) !important; }
        :global(.Toastify__close-button){ color: ${TEXT_MAIN} !important; opacity: .85; }
